...
Concretely the app surfaces: login/register pages, a dashboard, a project list and project detail/kanban view, task create/edit dialogs with assignee + priority + labels, and a comment thread on each task. The Next.js track ships the full UI; the FastAPI track ships the same features as a JSON API with auto-generated OpenAPI docs at `/docs`.
...
│   │   │   ├── projects/         # /api/projects + /[id] + nested labels/tasks
│   │   │   ├── tasks/            # /api/tasks + /[id]
...
│   └── jest.config.js            # jsdom env, path alias @/* → ./
│
...
**Testing** (pytest, asyncio_mode=auto, httpx client; conftest.py overrides `get_db`)
```bash
pytest                                       # full suite
...
Both tracks model the same entities and relationships. Authoritative schemas:

- Next.js: `nextjs/prisma/schema.prisma`
- FastAPI: `fastapi/app/models/*.py` (one model per file)

Entities: `User` (role: ADMIN/MEMBER/VIEWER) → owns `Project` (status: ACTIVE/ARCHIVED) → has `Task` (status: TODO/IN_PROGRESS/DONE, priority: LOW/MEDIUM/HIGH/URGENT) → has `Comment`. `Label` belongs to a `Project` and joins `Task` via `TaskLabel` (many-to-many). Status/priority/role are stored as plain strings (not DB enums) — keep the literal values aligned across tracks; the TypeScript union types in `nextjs/lib/types.ts` are the canonical list.
...
- UI: Tailwind + shadcn/ui primitives in `components/ui/`, feature components (`task-board.tsx`, `task-card.tsx`, `project-list.tsx`, `comment-thread.tsx`) at the top of `components/`.
...
- `app/main.py` builds the FastAPI app, mounts CORS, and includes the four routers (`auth`, `projects`, `tasks`, `comments`).
...
- Tests use `tests/conftest.py` fixtures (overrides `get_db` with an in-memory or test SQLite DB and provides an `httpx` client).
...
- The seed credentials (`admin123`, `password123`) are local dev fixtures. They must never appear in a production `.env`.
...
Seed data differs slightly between tracks — check the relevant README before assuming a login works. Next.js currently seeds `alice@example.com` / `password123` (and bob, charlie); FastAPI seeds `admin@taskforge.com` / `admin123` plus alice/bob/viewer.
